...
LeddarConnection::LdLibModbusSerial::Connect( void )
{
    ...
    try
    {
        ...
        modbus_t *lHandle = ( modbus_t * )mHandle;

        // Set slave address
        if( modbus_set_slave( lHandle, mConnectionInfoModbus->GetModbusAddr() ) != 0 )
        {
            modbus_free( ( modbus_t * )mHandle );
            mHandle = nullptr;
        ...
        lTimeout.tv_sec = 10;
        lTimeout.tv_usec = 0;
        modbus_set_response_timeout( ( modbus_t * )mHandle, &lTimeout );

        lTimeout.tv_sec = 0;
        lTimeout.tv_usec = 100000;
        modbus_set_byte_timeout( ( modbus_t * )mHandle, &lTimeout );
...
LeddarConnection::LdLibModbusSerial::SendRawRequest( uint8_t *aBuffer, uint32_t aSize )
{
    ...
    modbus_flush( ( modbus_t * )mHandle );

    // Set slave address
    if( modbus_set_slave( ( modbus_t * )mHandle, mConnectionInfoModbus->GetModbusAddr() ) != 0 )
    {
        throw LeddarException::LtConnectionFailed( "Connection failed, libmodbus errno: (" + LeddarUtils::LtStringUtils::IntToString( errno ) + std::string( "),  msg: " ) + std::string(
                    modbus_strerror(
                        errno ) ), true );
    ...
    int lResult = modbus_send_raw_request( ( modbus_t * )mHandle, aBuffer, aSize );
...
LeddarConnection::LdLibModbusSerial::ReadRegisters( uint16_t aAddr, uint8_t aNb, uint16_t *aDest )
{
    ...
    int lStatus = modbus_read_registers( ( modbus_t * )mHandle, aAddr, aNb, aDest );
...
LeddarConnection::LdLibModbusSerial::ReadInputRegisters( uint16_t aAddr, uint8_t aNb, uint16_t *aDest )
{
    ...
    int lStatus = modbus_read_input_registers( ( modbus_t * )mHandle, aAddr, aNb, aDest );
...
LeddarConnection::LdLibModbusSerial::WriteRegister( uint16_t aAddr, int aValue )
{
    ...
    int lStatus = modbus_write_register( ( modbus_t * )mHandle, aAddr, aValue );
...
LeddarConnection::LdLibModbusSerial::ReceiveRawConfirmation( uint8_t *aBuffer, uint32_t aSize )
{
    ...
    if( aSize )
    {
        lResult = modbus_receive_raw_confirmation_sizeEnd( ( modbus_t * )mHandle, aBuffer, aSize );
    ...
    {
        lResult = modbus_receive_raw_confirmation_timeoutEnd( ( modbus_t * )mHandle, aBuffer );
...
LeddarConnection::LdLibModbusSerial::ReceiveRawConfirmationLT( uint8_t *aBuffer, int aDeviceType )
{
    ...
    {
        lResult = modbus_receive_raw_confirmation_0x41_M16( ( modbus_t * )mHandle, aBuffer );
    }
    else if( aDeviceType == LtComLeddarTechPublic::LT_COMM_DEVICE_TYPE_VU8 )
    {
        lResult = modbus_receive_raw_confirmation_0x41_LeddarVu( ( modbus_t * )mHandle, aBuffer );
    }
    else
    {
