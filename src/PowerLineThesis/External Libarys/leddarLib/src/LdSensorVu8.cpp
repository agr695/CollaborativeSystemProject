...
LdSensorVu8::GetConfig()
{
    ...
    try
    {
        ...
        lIntProp->SetValue( 0, ( int32_t )100e6 / ( ( uint16_t * )lOutputBuffer )[0] );
...
// *****************************************************************************
// Function: LdSensorVu8::GetStates
//
...
LdSensorVu8::GetStates()
{
    ...
    GetResultStates()->GetProperties()->GetFloatProperty( LeddarCore::LdPropertyIds::ID_RS_SYSTEM_TEMP )->SetRawValue( 0, lTemperature );
}
...
LdSensorVu8::GetConstants()
{
    ...
    try
    {
        ...
        lIntProp->SetValue( 0, 1 << lProductDevInfo.mTempSensorScaleBits );
        lIntProp->SetClean();
    }
    catch( std::exception &e )
    {
