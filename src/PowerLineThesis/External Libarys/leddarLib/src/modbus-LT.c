...
"modbus_receive_raw_confirmation_0x41_M16" to get confirmation.
...
-> modbus_receive_raw_confirmation_0x41_M16
...
typedef enum {
    ...
    MSG_CONFIRMATION,
    /* Request message on the client side */
    MSG_CONFIRMATION_0x41_LEDDARVU,
    /* Request message on the client side */
    MSG_CONFIRMATION_0x41_M16
} msg_type_t;
    ...
    msg_type_t msg_type, int alength)
{
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        else if ((msg_type == MSG_CONFIRMATION_0x41_M16) && (function == 0x41)) {
...
initial request.
*/
int modbus_receive_raw_confirmation_0x41_M16(modbus_t *ctx, uint8_t *rsp)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return receive_msg_LT(ctx, rsp, MSG_CONFIRMATION_0x41_M16, 0);
}
